function fillInSenaraiKakitangan(senaraiKakitangan, jenisPermohonan) {
    ...
    senaraiKakitangan.forEach(element => {
        var content = '<li>' +
                      '<div class="row">' +
                      '<div class="nav-item col-11" onclick="fillInKedatangan('+element.id+','+"'"+ jenisPermohonan +"'"+','+element.permohonan_with_users.id_permohonan_baru+','+element.id+')" role="presentation"><a class="nav-link" id="'+ element.id +'" data-toggle="pill" href="#" role="tab" aria-controls="test1" aria-selected="true">'+element.name+'</a></div>' +
