$(document).ready(function(){
    ...
    showDatatable();
    $('#dashboardKSDT_paginate').addClass('mb-4');
})

function showDatatable(){
    var id_user = $('#userID').val()
                var dashboardKSDT = $('#dashboardKSDT').DataTable({
                dom: 'lrtip',
                destroy: true,
                ...
                serverSide: true,
            ajax: {
                url: "kerani-semakan-dashboard/"+id_user,
                type: 'GET',
        ...
        dashboardKSDT.on('draw.dt', function () {
            var info = dashboardKSDT.page.info();
            dashboardKSDT.column(0, { search: 'applied', order: 'applied', page: 'applied' }).nodes().each(function (cell, i) {
...
$("#selectJenisPermohonan").on('change',function(){
    ...
    if(statusPilihan == "lulus"){
        $('#dashboardKSDT').DataTable().columns(11).search(
            "0"
        ).draw();
